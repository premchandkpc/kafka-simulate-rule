...
fn eval_expr(expr: &Expr, body: &serde_json::Value) -> Result<serde_json::Value, String> {
    match expr {
        Expr::Field(path) => {
            resolve_field(body, path)
        }
        Expr::FnCall { name, args } => {
            let evaluated: Result<Vec<serde_json::Value>, String> = args.iter()
                .map(|a| eval_expr(a, body))
                .collect();
            let evaluated = evaluated?;
            let str_owned: Vec<String> = evaluated.iter()
        ...
        Expr::Number(n) => {
            Ok(serde_json::Value::Number(serde_json::Number::from_f64(*n)
...
fn set_field(body: &mut serde_json::Value, path: &str, value: serde_json::Value) -> Result<(), String> {
    let parts: Vec<&str> = path.split('.').collect();
    if parts.is_empty() {
    ...
    for i in 0..parts.len() - 1 {
        match current {
            serde_json::Value::Object(ref mut map) => {
                current = map.entry(parts[i].to_string())
...
fn call_builtin(name: &str, args: &[&str]) -> Result<serde_json::Value, String> {
    match name {
        ...
        "now" => Ok(serde_json::Value::String(chrono_now())),
        "lower" => {
        ...
        "length" => {
            let s = args.first().unwrap_or(&"");
            Ok(serde_json::Value::Number(serde_json::Number::from(s.len() as u64)))
        }
        "concat" => {
            Ok(serde_json::Value::String(args.concat()))
        }
        "base64" => {
            let s = args.first().unwrap_or(&"");
        ...
        "substring" => {
            let s = args.first().unwrap_or(&"");
            let start = args.get(1).and_then(|a| a.parse::<f64>().ok()).unwrap_or(0.0) as usize;
            let end = args.get(2).and_then(|a| a.parse::<f64>().ok()).unwrap_or(s.len() as f64) as usize;
...
fn chrono_now() -> String {
    ...
    format!("{:04}-{:02}-{:02}T{:02}:{:02}:{:02}.{:09}Z",
        ...
        1 + (days % 28) as u32,
        hours as u32, mins as u32, sec as u32, nanos)
}
...
pub fn eval_map_expression(expr_str: &str, body: &[u8]) -> Result<Vec<u8>, String> {
    if !expr_str.contains('=') {
        return Err("not an assignment expression (missing =)".to_string());
    }

    let eq_pos = expr_str.find('=').unwrap();
...
mod tests {
    ...
    #[test]
    fn test_field_path() {
        let body: serde_json::Value = serde_json::from_str(r#"{"user":{"name":"alice"}}"#).unwrap();
    ...
    fn test_function_replace() {
        ...
        let result = eval_map_expression("text=replace(text,'world','alice')", body).unwrap();
        let json: serde_json::Value = serde_json::from_slice(&result).unwrap();
