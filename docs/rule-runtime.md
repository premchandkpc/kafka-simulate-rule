...
**Implemented in:** `internal/rules/compiler.go`, `internal/rules/evaluator.go`
...
**Implemented in:** `internal/rules/compiler.go` - validates operators, paths, duplicate IDs, ambiguous priorities, and limits.
...
**Implemented in:** `internal/rules/evaluator.go` - `ComputeDecisionHash` function.

## Effect identity (implemented)
...
**Implemented in:** `internal/domain/types.go` - `ComputeEffectID` function.
...
**Implemented in:** `internal/rules/compiler.go` - `Limits` struct and validation.

| Limit | Default | Description |
| --- | --- | --- |
| MaxRulesPerSet | 100 | Maximum rules per rule set |
| MaxPredicates | 200 | Maximum predicates per rule |
| MaxNestingDepth | 10 | Maximum predicate nesting depth |
| MaxActionsPerSet | 10 | Maximum actions per rule |
| MaxPayloadBytes | 256KB | Maximum payload size |
...
**Implemented in:** `internal/domain/errors.go` - 50+ typed errors with `ErrorClass` classification.